...
void setpointCallback(const omnicopter_sim::RCInput& input){
	...
	if(input.rc_mode.position_control_mode == omnicopter_sim::ControlMode::MODE_CONTROL_FORCE_BODY_UP){
		msg.vector.x = 0;
		msg.vector.y = 0;
		msg.vector.z = (input.throttlestick+1)*5; // 0-6 N
	}
	else if(input.rc_mode.position_control_mode == omnicopter_sim::ControlMode::MODE_CONTROL_FORCE_BODY){
		msg.vector.x = input.pitchstick*1;// -3 - 3 N
		msg.vector.y = -input.rollstick*1; // -3 - 3 N
		msg.vector.z = (input.throttlestick+1)*5;
	}
	else if(input.rc_mode.position_control_mode == omnicopter_sim::ControlMode::MODE_CONTROL_FORCE_ENU){
